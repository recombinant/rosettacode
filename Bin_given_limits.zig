...
pub fn main() !void {
    ...
    const bins1 = fillBins(u8, &limits1, &data1);
    printBins(u8, &limits1, &bins1);

    print("Example 2:\n", .{});
    const limits2 = [_]u16{ 14, 18, 249, 312, 389, 392, 513, 591, 634, 720 };
    const data2 = [_]u16{
    ...
    const bins2 = fillBins(u16, &limits2, &data2);
    printBins(u16, &limits2, &bins2);
}
...
fn upperBound(T: type, array: []const T, value: T) usize {
    var lo: usize = 0;
...
fn printBins(T: type, limits: []const T, bins: []const u32) void {
    print("          < {d:3} := {d:3}\n", .{ limits[0], bins[0] });

    for (1..limits.len) |i|
        print(">= {d:3} .. < {d:3} := {d:3}\n", .{ limits[i - 1], limits[i], bins[i] });

    print(">= {d:3}          := {d:3}\n\n", .{ limits[limits.len - 1], bins[bins.len - 1] });
}
