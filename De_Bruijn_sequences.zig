...
pub fn main(init: std.process.Init) !void {
    ...
    defer de_bruijn_list.deinit(gpa);
    ...
    for (0..100) |n| {
        ...
        if (a2 >= a1) {
            if (a1 == a2)
                try de_bruijn_list.append(gpa, a1)
            else
                try de_bruijn_list.appendSlice(gpa, a);
            var m = n + 1;
            while (m <= 99) : (m += 1) {
                ...
                if (ms[1] > a1) {
                    try de_bruijn_list.appendSlice(gpa, a);
                    try de_bruijn_list.appendSlice(gpa, ms);
                }
            }
        }
    }
    try de_bruijn_list.appendSlice(gpa, de_bruijn_list.items[0..3]);
    var de_bruijn = try de_bruijn_list.toOwnedSlice(gpa);
    defer gpa.free(de_bruijn);
    ...
    const result3 = try check(gpa, de_bruijn);
    print("{s}", .{result3});
...
fn check(allocator: Allocator, text: []const u8) ![]const u8 {
