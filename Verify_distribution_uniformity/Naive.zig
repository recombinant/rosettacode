...
pub fn main(init: std.process.Init) !void {
    ...
    while (cnt <= 1000000) : (cnt *= 10) {
        ...
        try stdout.writeAll(if (try check(gpa, random, rand5_7, 7, cnt, 0.03, stdout)) "flat\n" else "NOT flat\n");
    }
...
/// `delta` is relative
fn check(allocator: Allocator, random: std.Random, gen: *const fn (std.Random) u4, n: usize, cnt: usize, delta: f64, writer: *std.Io.Writer) !bool {
