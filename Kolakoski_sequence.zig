...
pub fn main(init: std.process.Init) !void {
    ...
    for (ias, slens) |ia, slen| {
        const kol = try kolakoski(gpa, ia, slen);
        defer gpa.free(kol);
        try stdout.print("First {d} members of the sequence generated by: {any}\n", .{ slen, ia });
        try stdout.print("{any}\n", .{kol});
        const p = try isPossibleKolakoski(gpa, kol);
...
fn kolakoski(allocator: Allocator, cycle: []const usize, slen: usize) ![]usize {
...
fn isPossibleKolakoski(allocator: Allocator, sequence: []usize) !bool {
